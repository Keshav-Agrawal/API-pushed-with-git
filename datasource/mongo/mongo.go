...
func (m mongoDS) UpdateOneTask(workId string) (string, error) {
	...
	if err != nil {
		...
		return (workId), (errors.New("updation failure"))
	}
...
func (m mongoDS) DeleteOneTask(workId string) (string, error) {
	id, _ := primitive.ObjectIDFromHex(workId)
	filter := bson.M{"_id": id}
	...
	if err != nil {
		...
		return (workId), (errors.New("deletion failure"))
	}
	fmt.Println("task got delete with delete count: ", deleteCount)
	return workId, nil
}

func (m mongoDS) DeleteAllTask() (int64, error) {

	deleteResult, err := m.c.DeleteMany(context.Background(), bson.D{{}}, nil)

	if err != nil {
		log.Println("Deletealltask::error::", err)
		return 5, (errors.New("deletionall failure"))
	}
